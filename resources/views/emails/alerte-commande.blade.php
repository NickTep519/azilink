...
<body>
    ...
    <div class="container">
        ...
        <table width="100%" style="border-collapse: collapse;">
            <tr>
                <td widdth="50%" style="background:#eee;padding:20px;">
                    <strong>Date:</strong> {{ \Carbon\Carbon::now()->translatedFormat('l d F Y') }} <br>
                </td>
                <td style="background:#eee;padding:20px;">
                    <strong>Commande :</strong> {{ $commande->annonce?->title }} <br>
                    <strong>Createur de la commande :</strong> {{ $commande->creatorUser?->first_name }} {{ $commande->creatorUser?->name }} <br>
                    <strong>Receveur de la commande :</strong> {{ $commande->receverUser?->first_name }} {{ $commande->receverUser?->name }} <br>
                </td>
            </tr>
        </table><br>
        <table width="100%">
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                ...
                                {{ $commande->annonce?->company }}
                                {{ $commande->annonce?->title }}<br>
                                {{ $commande->annonce?->starts_city }} <br>
                                {{ $commande->annonce?->ends_city }} <br>
                            </td>
                        </tr>
                    </table>
                </td>
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                ...
                                Depart : {{ $commande->annonce?->starts_at->translatedFormat('d M Y') }} <br>
                                Arrivée {{ $commande->annonce?->ends_at->translatedFormat('d M Y') }} <br>
                            </td>
                        </tr>
                    </table>
                </td>
        ...
        <table width="100%" style="border-collapse: collapse;border-bottom:1px solid #eee;">
            ...
            <tr>
                <td class="row"><span style="color:#777;font-size:11px;"></span><br> {{ $commande->title }} </td>
                <td class="row"> {{ $commande->kg_commande }} kg</td>
                <td class="row"> {{ $commande->price }}  €</td>
            </tr>
        ...
        <div class="socialmedia"> L'Equipe <small> <a href="{{ route('home') }}"> {{ config('app.name') }} </a></small></div>
    </div><!-- container -->
